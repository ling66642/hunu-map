...
export default function CampusMapApp(){
  ...
  useEffect(()=>{
    ...
    Promise.all([
      fetch(assetUrl('/data/erliban_boundary.geojson')).then(r=>r.json()),
      fetch(assetUrl('/data/erliban_buildings.geojson')).then(r=>r.json()),
      fetch(assetUrl('/data/erliban_roads.geojson')).then(r=>r.json()),
      fetch(assetUrl('/data/erliban_water.geojson')).then(r=>r.json())
  ...
  return <div className="standalone-map-app">
    <header className="map-topbar">
      <a className="map-brand" href={assetUrl('/')}><img src={assetUrl('/images/师大校徽.webp')} alt="湖南师范大学校徽" className="map-seal" /><span><strong>湖南师范大学</strong><small>二里半校园地图</small></span></a>
      <div className="map-top-title"><MapPin size={16}/><span>二里半校区</span><em>75栋建筑 · 专属矢量底图</em></div>
      <nav className="map-view-switch" aria-label="地图模式切换"><a className="active" href={assetUrl('/map.html')}><MapPin size={14}/>交互地图</a><a href={assetUrl('/poster.html')}><Camera size={14}/>静态成图</a></nav>
      <a className="back-home" href={assetUrl('/')}><ChevronLeft size={17}/>返回导览首页</a>
